...
def conv2d(x, filters, size, pad=0, stride=1, batch_normalize=1,
	...
	#y=γ(x-μ)/σ+β  其中x是输入，y是输出，μ是均值，σ是方差，γ和β是缩放（scale）、偏移（offset）系数。
	#保存训练参数时要注意u、δ
    if activation:
        out = activation(out)
    return out
...
#ksize:滑动窗口的大小，长度必须大于四  strides rates（每个patch中取点的间隔） padding:VALID”或者“SAME"same补零
...
def darknet(images, n_last_channels=425):
    ...
    net = conv2d(net, 512, 1, name="conv6_2")            #使用1x1卷积压缩特征
